#!/usr/bin/env python3
"""Run one fresh-process Phase-4 buffer-boundary correctness profile."""
...
from run_dirtygen_perf import (
    command_record, find_repo_root, finish_metadata, now, repository_states,
    toolchain_information, top_level_gitlinks, trace_signature, write_metadata,
)


IDENTIFIER = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]*\Z")
SCHEMA = "shdlt-buffer-boundary-phase4-correctness-run-v1"
...
def profile_id(harts: int) -> int:
    return 1 if harts == 1 else 2


def elf_path(root: Path, harts: int) -> Path:
    return (dirtygen_dir(root) / "build" / f"buffer-boundary-v1-h{harts}" /
            "dirtygen_buffer_boundary_mc.elf")


def build_command(root: Path, harts: int) -> list[str]:
    target = "buffer-boundary-h1" if harts == 1 else "buffer-boundary-mc"
    result = ["make", "-C", str(dirtygen_dir(root)), target]
    if harts != 1:
        result.append(f"BOUNDARY_MC_HARTS={harts}")
    return result
...
def cpu_config(harts: int, mode: str) -> dict[str, Any]:
    return {
        ...
        "stdin": False, "rvls_check": mode == "rvls",
...
def simulation_name(experiment: str, harts: int, mode: str,
                    run_id: str) -> str:
    return f"shdlt_buffer_boundary_{experiment}_h{harts}_{mode}_seed2_{run_id}"
...
def mill_command(root: Path, experiment: str, harts: int, mode: str,
    ...
    if mill is None:
        raise RunFailure("environment", "mill was not found in PATH")
    ...
    command.extend([
        "--performance-counters", "4", "--load-elf", str(elf_path(root, harts)),
        "--pass-symbol", "pass", "--fail-symbol", "fail", "--pass-policy", "all",
        "--fail-policy", "any", "--fail-after", str(config["fail_after"]),
        "--dbus-ready-factor", str(config["dbus_ready_factor"]),
        "--memory-latency", "0", "--seed", "2", "--name",
        simulation_name(experiment, harts, mode, run_id), "--with-rvls-log",
    ])
...
def report_command(root: Path, console: Path, tracer: Path, output: Path,
    ...
    return [
        sys.executable,
        str(dirtygen_dir(root) / "tools" / "dirtygen_buffer_boundary_report.py"),
        str(console), "--output-dir", str(output), "--hart-count", str(harts),
        "--profile", str(profile_id(harts)), "--tracer", str(tracer),
        "--elf", str(elf_path(root, harts)),
    ]
...
def symbols(elf: Path) -> dict[str, int]:
    wanted = {
        "boundary_mc_guest_start", "boundary_mc_guest_end", "boundary_mc_store",
        "boundary_mc_epoch_start", "boundary_mc_epoch_end", "pass", "fail",
    }
    try:
...
def artifact_fingerprint(elf: Path) -> dict[str, Any]:
    ...
    start = symbol_map["boundary_mc_guest_start"]
    end = symbol_map["boundary_mc_guest_end"]
    ...
    store = symbol_map["boundary_mc_store"] - start
    if store < 0 or store + 4 > payload_size:
        raise RunFailure("artifact", "guest store symbol lies outside payload")
    return {
        ...
        "symbols": {name: f"0x{value:016x}" for name, value in sorted(symbol_map.items())},
        "guest_runtime_pc": {"store": f"0x{store:x}"},
    }


def fingerprints(root: Path, paths: list[str]) -> list[dict[str, str]]:
...
def source_fingerprints(root: Path) -> list[dict[str, str]]:
    return fingerprints(root, [
        ...
        "ext/NaxSoftware/benchmarks/dirtygen/tools/run_dirtygen_buffer_boundary_phase4.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = args.output_root or (
        ...
        f"{args.experiment_id}-h{args.hart_count}-{args.mode}-seed2")
    build = build_command(root, args.hart_count)
    simulation = mill_command(root, args.experiment_id, args.hart_count,
                              args.mode, run_id)
    name = simulation_name(args.experiment_id, args.hart_count, args.mode, run_id)
    console = output / "console.log"
    archived_trace = output / "tracer.log"
    report_dir = output / "report"
    if args.dry_run:
        print(json.dumps({
            "scope": f"correctness profile H{args.hart_count} {args.mode}",
        ...
        return 0
    if output.exists():
        print(f"buffer-boundary runner error: output exists: {output}", file=sys.stderr)
        return 2
    output.mkdir(parents=True)
    metadata_path = output / "metadata.json"
    raw_trace = trace_path(root, name)
    protected_before = protected_fingerprints(root)
    source_before = source_fingerprints(root)
    ...
    metadata: dict[str, Any] = {
        ...
        "scope": f"buffer-boundary correctness H{args.hart_count} {args.mode}",
        "profile": profile_id(args.hart_count), "hart_count": args.hart_count,
        "mode": args.mode, "simulation_seed": 2, "fresh_process": True,
        "host_timeout_seconds": 1800, "trace_required": True,
        "trace_generated": False, "trace_path": None,
        "generated_rtl": None,
        "cpu_config": cpu_config(args.hart_count, args.mode),
        "repositories": repositories, "source_files": source_before,
        "protected_before": protected_before, "normative": normative_fingerprints(),
        ...
        "commands": {"build": command_record(build),
                     "simulation": command_record(simulation), "report": None},
    ...
    try:
        ...
        shutil.copy2(assembly, output / "dirtygen_buffer_boundary_mc.asm")
        write_metadata(metadata_path, metadata)
        ...
        generated_rtl = generated_rtl_path(root)
        if not generated_rtl.is_file():
            raise RunFailure("artifact", f"generated RTL was not found: {generated_rtl}")
        archived_rtl = output / "TestBenchDut.generated.v"
        shutil.copy2(generated_rtl, archived_rtl)
        metadata["generated_rtl"] = {
            "path": archived_rtl.name,
            "sha256": sha256_file(archived_rtl),
            "bytes": archived_rtl.stat().st_size,
        }
        ...
        report = report_command(root, console, archived_trace, report_dir,
                                args.hart_count)
        ...
        metadata["firmware_verdict"] = load_json(report_dir / "verdict.json")
        metadata["protected_after"] = protected_fingerprints(root)
        metadata["protected_after_matches"] = metadata["protected_after"] == protected_before
        metadata["source_after"] = source_fingerprints(root)
        metadata["source_after_matches"] = metadata["source_after"] == source_before
        write_metadata(metadata_path, metadata)
        if simulation_code:
            raise RunFailure("simulation",
                             f"simulation exited {simulation_code}; verification failed or incomplete",
                             simulation_code)
        if report_code:
        ...
        write_metadata(metadata_path, metadata)
        print(f"buffer-boundary runner error: {error}", file=sys.stderr)
        return error.code

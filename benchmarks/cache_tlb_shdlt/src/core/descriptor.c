...
static const struct ctc_case_descriptor cases[CTC_CASE_COUNT] = {
  ...
  /* Each epoch starts with an empty valid log, independently of optional
     deduplication. GPA/VMID over-fencing is architecturally permitted. */
  {5, 1, 3, 1, 1, CTC_OBS_TLB | CTC_OBS_PTE, CTC_PASS},
  {6, 2, 2, 0, 0, CTC_OBS_TLB, CTC_PASS},
  {7, 2, 2, 0, 0, CTC_OBS_TLB, CTC_PASS},
#else
